...
describe('getDaysInMonth', () => {
  ...
  it('유효하지 않은 월에 대해 적절히 처리한다', () => {
    expect(getDaysInMonth(2024, 0)).toBe(31); // 이전 해의 12월
    expect(getDaysInMonth(2024, 13)).toBe(31); // 다음 해의 1월
  });
});

describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const date = new Date('2024-02-14'); // 수요일
    const weekDates = getWeekDates(date);

    expect(weekDates).toHaveLength(7);
    expect(weekDates[0].getDate()).toBe(11); // 일요일
    expect(weekDates[6].getDate()).toBe(17); // 토요일
  });

  it('주의 시작(일요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const date = new Date('2024-02-11'); // 일요일
    const weekDates = getWeekDates(date);

    expect(weekDates[0].getDate()).toBe(11);
    expect(weekDates[6].getDate()).toBe(17);
  });

  it('주의 끝(토요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const date = new Date('2024-02-17'); // 토요일
    const weekDates = getWeekDates(date);
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연말)', () => {
    const date = new Date('2024-12-29'); // 2024년 마지막 주 일요일
    const weekDates = getWeekDates(date);
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연초)', () => {
    const date = new Date('2024-01-06'); // 2024년 첫 주 토요일
    const weekDates = getWeekDates(date);
...
describe('getWeeksAtMonth', () => {
  it('2024년 7월 1일의 올바른 주 정보를 반환해야 한다', () => {
    ...
    expect(weeks).toHaveLength(5); // 7월은 5주
    expect(weeks[0][0]).toBe(null); // 첫 주 월요일은 비어있음
    expect(weeks[0][1]).toBe(1); // 7월 1일은 화요일
  });
});
